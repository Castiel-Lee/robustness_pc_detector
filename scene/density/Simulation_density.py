...
from Modify_density import opr_pc as corrupting_scan
import time
...
dir_save =join(dir_save,'velodyne_'+noise_model+'_'+str(severity))
...
for (n, path_scan) in enumerate(pc_list):
    if end_pc > n >=star_pc:
        corrupting_scan(path_pc=path_scan, dir_save=dir_save, corruption=noise_model, saving=True, severity=severity)
        now=time.time()
        exp_rem_tim = (now - start) / (n + 1 - star_pc) * (N_pc  - (n + 1))
